...
public class SShape extends Shapes {
	...
	public SShape(int w, int h) {
		super(w,h);
		color = getRandomColor();
		if (rotation == 1) {
			a1= getRandomNumber(gameFieldWeight-1); // Если вертикальное положение
			a2=a1+gameFieldWeight;
			a3=a2+1;
			a4=a3+gameFieldWeight;
		} else if (rotation == 0){ // Если горизонтальное положение
			a1= getRandomNumber(gameFieldWeight-maxLength+1)+2;
			a2=a1-1;
			a3=a2+gameFieldWeight;
			a4=a3-1;
		}
